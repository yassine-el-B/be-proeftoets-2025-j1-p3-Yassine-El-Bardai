...
class Core 
{
    ...
    public function __construct()
    {
         ...
         */
        if ($this->currentController == 'Zangeres') {
            require_once '../app/models/ZangeresModel.php';
            $database = new Database(); // Zorg ervoor dat je een Database-object hebt
            $zangeresModel = new ZangeresModel($database);
            $this->currentController = new $this->currentController($zangeresModel);
        } elseif ($this->currentController == 'Speakers') {
            require_once '../app/models/SpeakersModel.php';
            $database = new Database(); // Zorg ervoor dat je een Database-object hebt
            $speakersModel = new SpeakersModel($database);
            $this->currentController = new $this->currentController($speakersModel);
        } elseif ($this->currentController == 'Bucketlist') {
            require_once '../app/models/BucketlistModel.php';
            $database = new Database(); // Zorg ervoor dat je een Database-object hebt
